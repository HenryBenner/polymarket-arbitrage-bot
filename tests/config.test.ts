...
test("ladder_v11 is BTC-only Kalshi paper/live mode with existing live safeguards", () => {
  const config = testConfig({
    exchange: "kalshi",
  ...
  assert.throws(
    () => validateTradingConfig({ ...config, kalshiSeriesTickers: ["KXETH15M"] }),
    /ladder_v11 is BTC-only/,
  );
  assert.throws(
    () => validateTradingConfig({ ...config, minutesBeforeCloseMax: 10 }),
    /full BRTI observation window/,
  );
});

test("ladder_v12 is BTC-only Kalshi paper/live mode with existing live safeguards", () => {
  const config = testConfig({
    exchange: "kalshi",
...
test("ladder_v13 is full-window BTC-only Kalshi paper/live mode", () => {
  ...
  assert.throws(
    () => validateTradingConfig({ ...config, kalshiSeriesTickers: ["KXETH15M"] }),
    /ladder_v13 is BTC-only/,
  );
  assert.throws(
    () => validateTradingConfig({ ...config, minutesBeforeCloseMax: 10 }),
    /full 15-minute market window/,
  );
});
